			...
			for (int x = 0; x <= this._fp.xFloorPlanDim() - 1; x++) {
				...
				if (_tmpFC != null) {
					...
					if (_tmpFC.getFloorType() == FloorTypes.BARE)
						...
						alpha = 200;
					if (_tmpFC.getCoordinates().equals(
							this._fp.getChargingStation().getCoordinates())) {
						// charger
						g2d.setColor(new Color(21, 0, 255));
						g2d.fillRect(intialPosition + (x * squareWidth),
								intialPosition + (y * squareWidth),
								squareWidth, squareWidth);

					}
					if (this._r.getCoordinates()
							.equals(_tmpFC.getCoordinates())) {

						// robot
						g2d.drawImage(image,
								intialPosition + (x * squareWidth),
								intialPosition + (y * squareWidth), this);
					} else {
